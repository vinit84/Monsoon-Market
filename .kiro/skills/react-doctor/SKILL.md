---
name: react-doctor
description: Diagnose and fix React codebase health issues. Use when reviewing React code, auditing effect-heavy components, fixing performance problems, auditing security, or improving code quality.
version: 1.0.0
---
...
Treat direct `useEffect` as a smell or anti-pattern by default. Prefer render-time derivation, event handlers, keyed resets, and data-fetching abstractions unless the component is synchronizing with an external system such as the DOM, a subscription, a timer, or a third-party widget.
...
- **State & Effects**: direct `useEffect` as default control flow, derived state in useEffect, event-to-effect action relays, effect-based fetch choreography, missing cleanup, useState from props, cascading setState
