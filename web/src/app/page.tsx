...
import { useLiveRequest } from "@/hooks/use-live-request";
import { Badge } from "@/components/ui/badge";

export default function Home() {
    // Derived from the SSE event stream — works on Vercel because we don't
    // depend on the per-lambda in-memory `_requests` array.
    const latest = useLiveRequest();
    ...
    const auctionDecided =
        status === "awarded" || status === "fulfilled" || status === "disputed";
                ...
                {latest && (
                    <Card>
                        <CardHeader
                            ...
                            action={
                                <Badge variant="brass">{latest.bountyMon.toFixed(3)} MON BOUNTY</Badge>
                            }
                        />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[13px]">
                            {latest.category && (
                                <div>
                                    <div className="text-[10px] uppercase tracking-widest text-[color:var(--color-ink-muted)] mb-1">
                                        Category
                                    </div>
                            ...
                            {latest.winner && (
                                <div className="md:col-span-2">
                                    ...
                                    <div>
                                        <strong>{latest.winner.agentLabel}</strong> ·{" "}
                                        {latest.winner.priceMon.toFixed(4)} MON · ETA{" "}
                                        {latest.winner.etaSeconds}s
                                    </div>
                                </div>
                            )}
                        </div>
